...
function formatTime(minutes, seconds) {
  const changedMinutes = String(minutes).padStart(2, '0');
  const changedSeconds = String(seconds).padStart(2, '0');
...
function isPalindrome(str) {
  const stringWithoutChar = str.replace(/[^a-zA-Z]/g, '').toLocaleLowerCase();
  const reversStr = stringWithoutChar.split('').reverse().join('');
  return reversStr === stringWithoutChar;
}
...
function reverseWords(str) {
  const arr = str.split(' ');
  const arrayReversedWords = arr.map((i) => i.split('').reverse().join(''));
  return arrayReversedWords.join(' ');
}
